...
export default function ScoutPage() {
  ...
  const avgBreakoutPotential = Math.round(
    breakoutRadarRows.reduce((sum, r) => sum + r.breakoutProbability, 0) /
      Math.max(1, breakoutRadarRows.length)
  )
                ...
                <div className="p-5 border-b border-border flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Emerging artists with strong breakout signals.
                    </p>
                  </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">TikTok usage growth</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Shazam spike level</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Mentions velocity</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Playlist growth</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Breakout Probability</th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      <div className="mt-4 md:hidden divide-y divide-border rounded-2xl border border-border overflow-hidden bg-background/20">
                        {filteredArtists.map((artist) => (
                          <Link
                            ...
                            className="block p-4 hover:bg-muted/20 transition"
                          >
                            <div className="flex items-center justify-between gap-3">
                ...
                <div className="grid grid-cols-1 gap-6">
                  <div className="card-glow p-5">
                    <div className="flex items-center gap-3 mb-4">
                      <img src={selectedA.image} alt={selectedA.name} className="h-10 w-10 rounded-full object-cover border border-border bg-muted" />
                      <div className="min-w-0">
                        <p className="font-bold truncate">{selectedA.name}</p>
                        <p className="text-xs text-muted-foreground truncate">{selectedA.country} • {selectedA.genre[0]}</p>
                      </div>
                    </div>
                  ...
                  <div className="card-glow p-5">
                    <div className="flex items-center gap-3 mb-4">
                      <img src={selectedB.image} alt={selectedB.name} className="h-10 w-10 rounded-full object-cover border border-border bg-muted" />
                      <div className="min-w-0">
                        <p className="font-bold truncate">{selectedB.name}</p>
                        <p className="text-xs text-muted-foreground truncate">{selectedB.country} • {selectedB.genre[0]}</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
